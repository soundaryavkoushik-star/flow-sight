...
function ForecastBriefing({ expanded = false }: { expanded?: boolean }) {
  return <div className={`relative overflow-hidden rounded-[28px] border border-[#D7E0EC] bg-white shadow-[0_30px_90px_rgba(15,29,58,0.13)] ${expanded ? "p-5 sm:p-7" : "p-4 sm:p-5"}`}>
    <div className="flex items-center justify-between border-b border-[#E2E5EB] pb-4">
      <div><p className="text-[10px] uppercase tracking-[0.15em] text-[#6B7280]" style={mono}>Your 30-day outlook</p><p className="mt-1 text-sm font-medium text-[#0F1D3A]">Today → August 20</p></div>
    ...
    <div className={`grid gap-4 pt-5 ${expanded ? "md:grid-cols-[0.78fr_1.22fr]" : "sm:grid-cols-2"}`}>
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-[#F2F6FC] p-4"><p className="text-[10px] text-[#6B7280]">Current balance</p><p className="mt-2 text-xl font-medium text-[#0F1D3A]" style={mono}>$4,260</p></div>
        <div className="rounded-2xl bg-[#2D8B5A]/[0.08] p-4"><p className="text-[10px] text-[#6B7280]">Safe to Spend</p><p className="mt-2 text-xl font-medium text-[#2D8B5A]" style={mono}>$680</p></div>
        <div className="col-span-2 rounded-2xl border border-[#D7E0EC] p-4"><div className="flex items-center justify-between"><div><p className="text-[10px] text-[#6B7280]">Projected low</p><p className="mt-1 text-lg font-medium text-[#C9633B]" style={mono}>$420</p></div><div className="text-right"><p className="text-[10px] text-[#6B7280]">When</p><p className="mt-1 text-sm font-medium">August 3</p></div></div><p className="mt-3 text-xs leading-relaxed text-[#6B7280]">Rent, insurance, and your car payment arrive before payday.</p></div>
      </div>
      <div className="rounded-2xl border border-[#D7E0EC] bg-[#F7FAFF] p-4">
        <div className="mb-2 flex justify-between text-[9px] text-[#6B7280]"><span>Projected balance</span><span>30 days</span></div>
        <svg viewBox="0 0 484 138" className="h-[170px] w-full overflow-visible" role="img" aria-label="Forecast falling to 420 dollars before recovering">
          ...
          <polyline points={forecastPoints} fill="none" stroke="#C9633B" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="fs-hero-sample-line" />
          <circle cx="264" cy="113" r="6" fill="#C9633B" stroke="white" strokeWidth="3" className="fs-hero-sample-point" />
          <text x="270" y="130" fill="#C9633B" fontSize="9">Aug 3 · $420</text>
        </svg>
...
function HeroCopy() {
  return <div>
    <p className="mb-6 text-xs font-medium uppercase tracking-[0.15em] text-[#C9633B]" style={mono}>Cash flow, with context</p>
    <h1 className="max-w-[620px] text-[48px] font-medium leading-[1.02] tracking-[-0.02em] text-[#0F1D3A] sm:text-[62px]" style={display}>Know what your money <span className="text-[#C9633B]">does next.</span></h1>
    ...
    <div className="mt-8 flex flex-wrap gap-3"><button className="inline-flex items-center gap-2 rounded-xl bg-[#C9633B] px-5 py-3 text-sm font-medium text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#B75532]">Join the Beta <ArrowRight size={15} /></button><button className="rounded-xl border border-[#D7E0EC] bg-white px-5 py-3 text-sm font-medium text-[#0F1D3A] transition hover:border-[#C9633B]/35">See how it works</button></div>
    <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-xs text-[#6B7280]">{["No bank connection required", "Transparent calculations"].map((item) => <span key={item} className="inline-flex items-center gap-2"><CheckCircle2 size={13} className="text-[#2D8B5A]" />{item}</span>)}</div>
  </div>
}
  ...
  return <section ref={sectionRef} className="relative h-[190vh] bg-[radial-gradient(circle_at_12%_10%,rgba(138,181,226,0.25),transparent_32rem),linear-gradient(145deg,#F7FAFF,#EEF4FB_58%,#FFF7F2)]">
    <div className="sticky top-0 flex min-h-screen flex-col justify-center overflow-hidden px-5 py-20">
      <div className="mx-auto w-full max-w-[1240px]">
        ...
        <div className="pointer-events-none mx-auto mt-5 flex items-center justify-center gap-2 text-[10px] uppercase tracking-[0.15em] text-[#6B7280] transition-opacity" style={{ opacity: 1 - progress * 2 }}><ChevronDown size={13} />Scroll to expand the forecast</div>
        <div className="pointer-events-none mx-auto -mt-4 max-w-xl text-center transition-[opacity,transform]" style={{ opacity: Math.max(0, (progress - 0.62) * 2.7), transform: `translateY(${18 - progress * 18}px)` }}><p className="text-xs font-medium uppercase tracking-[0.15em] text-[#C9633B]" style={mono}>Five days of warning</p><h2 className="mt-3 text-3xl font-medium text-[#0F1D3A]" style={display}>See it <span className="text-[#C9633B]">before it arrives.</span></h2></div>
      </div>
    </div>
  </section>
}
