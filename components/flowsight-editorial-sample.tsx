...
function HeroPreview({ mode }: { mode: number }) {
  ...
  return <div className="relative rounded-[28px] bg-[#EFE5D8] p-4 shadow-[0_30px_80px_rgba(74,65,60,.12)] sm:p-6">
    <div className="rounded-[20px] border border-[#DCCBBA] bg-[#FFFDFC] p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs text-[#756A63]">Your next 30 days</p><h3 className="mt-1 font-editorial text-3xl font-normal text-[#4A413C]">What happens next</h3></div><span className={`rounded-full px-3 py-1.5 text-xs ${item.condition === "Clear" ? "bg-[#E8F4ED] text-[#2D8B5A]" : "bg-[#F9F0DD] text-[#9A6719]"}`}>{item.condition}</span></div>
      <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-[#F7F2EA] p-3"><p className="text-[11px] text-[#756A63]">Current balance</p><p key={`balance-${mode}`} className="mt-2 font-mono text-xl text-[#4A413C] motion-safe:animate-[number-roll_.55s_cubic-bezier(.19,1,.22,1)_both]">{item.balance}</p></div><div className="rounded-xl bg-[#EFE5D8] p-3"><p className="text-[11px] text-[#756A63]">Safe to Spend</p><p key={`safe-${mode}`} className="mt-2 font-mono text-xl text-[#BB6C43] motion-safe:animate-[number-roll_.55s_cubic-bezier(.19,1,.22,1)_both]">{item.safe}</p></div></div>
...
function AhaDiagram() {
  ...
  return <div ref={cue.ref} className="rounded-[28px] border border-[#DCCBBA] bg-[#FFFDFC] p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs text-[#756A63]">Example forecast · Jul 21 → Aug 3</p><p className="mt-1 font-editorial text-2xl text-[#4A413C]">Five days of warning</p></div><span className="rounded-full bg-[#F9F0DD] px-3 py-1.5 text-xs text-[#9A6719]">Watch · Aug 3</span></div>
...
function ScenarioPanel() {
  ...
  return <div ref={cue.ref} className="rounded-[28px] bg-[#4A413C] p-5 text-white sm:p-8"><div className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-center"><div><div className="space-y-2">{scenarioChoices.map((item,index)=><div key={item.label} className={`w-full rounded-xl border px-4 py-3 text-left text-sm transition-all duration-500 ${choice===index?"translate-x-2 border-[#E5CBB9] bg-[#C8906D]":"border-white/15 text-white/45"}`}>{item.label}{item.amount>0&&<span className="float-right font-mono">−${item.amount.toLocaleString()}</span>}</div>)}</div></div><div className="rounded-2xl bg-[#FFFDFC] p-5 text-[#4A413C]"><div className="flex justify-between gap-4"><div><p className="text-xs text-[#756A63]">With {current.label.toLowerCase()}</p><p className="mt-1 font-editorial text-2xl">Safe to Spend · <span key={safe} className="inline-block text-[#BB6C43] motion-safe:animate-[number-roll_.55s_cubic-bezier(.19,1,.22,1)_both]">${safe.toLocaleString()}</span></p></div><span className={`h-fit rounded-full px-3 py-1 text-xs transition-colors ${safe<1000?'bg-[#F9F0DD] text-[#9A6719]':'bg-[#E8F4ED] text-[#2D8B5A]'}`}>{safe<1000?'Watch':'Clear'}</span></div><ForecastLine path={path}/><div className="border-t border-[#DCCBBA] pt-3 text-xs text-[#625852]">This temporary scenario leaves a projected low of <span className="font-mono font-medium text-[#4A413C]">${(safe+500).toLocaleString()}</span>.</div></div></div></div>
}
...
function ProcessDiagram() {
  ...
  return <div ref={cue.ref} className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div className="space-y-2">{["01 · Bring your numbers","02 · Check what we found","03 · See what’s ahead"].map((label,index)=><div key={label} className={`w-full border-l-2 px-5 py-4 text-left transition-all duration-500 ${step===index?'border-[#C8906D] bg-[#F4E8DE] text-[#4A413C]':'border-[#DCCBBA] text-[#756A63]'}`}>{label}</div>)}</div><div className="relative min-h-[300px] overflow-hidden rounded-[28px] border border-[#DCCBBA] bg-[#FFFDFC] p-7"><span className="font-mono text-[10px] uppercase tracking-[.14em] text-[#C8906D]">Step 0{step+1}</span><div key={step} className="motion-safe:animate-[editorial-rise_.55s_ease-out_both]"><Icon className="mt-8 h-8 w-8 text-[#BB6C43]"/><h3 className="mt-5 font-editorial text-3xl text-[#4A413C]">{content.title}</h3><p className="mt-2 text-sm text-[#625852]">{content.copy}</p></div><div className="absolute bottom-7 left-7 right-7 flex items-center gap-2">{[0,1,2,3,4].map((n)=><span key={n} className={`h-10 flex-1 rounded-t border border-[#DCCBBA] transition-colors duration-500 ${n<=step+1?'bg-[#BB6C43]':'bg-[#FFFDFC]'}`} style={{height:26+n*8}}/>)}<ArrowRight className="ml-2 h-5 w-5 text-[#C8906D]"/></div></div></div>
}
...
export default function FlowSightEditorialSample(){
  ...
  return <main className="editorial-sample min-h-screen overflow-hidden bg-[radial-gradient(circle_at_82%_4%,rgba(200,144,109,.10),transparent_25rem),linear-gradient(145deg,#FFFAF4_0%,#F7F2EA_52%,#EFE5D8_100%)] font-sans text-[#4A413C] [--font-editorial:var(--sample-editorial)] [--font-mono:var(--sample-mono)] [--font-sans:var(--sample-sans)]">
    ...
    <footer className="border-t border-[#DCCBBA] px-5 py-10"><div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 text-sm text-[#756A63]"><span className="font-medium text-[#4A413C]">↗ FlowSight</span><span>Editorial sample · production landing unchanged</span><Link href="/design-sample" className="hover:text-[#BB6C43]">Previous sample</Link></div></footer>
  </main>
}
