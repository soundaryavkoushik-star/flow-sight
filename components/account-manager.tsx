...
function AccountEditor({ account, paymentAccounts, onClose, onSaved }: { account: ManagedAccount | "new"; paymentAccounts: PaymentAccount[]; onClose: () => void; onSaved: (saved: { isNew: boolean; accountId: string }) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="account-editor-title"><button className="absolute inset-0 bg-background/75 backdrop-blur-sm" onClick={onClose} aria-label="Close account editor" /><form className="relative w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto" onSubmit={async (event) => {
    ...
    {type === "credit_card" && isNew && <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-4">
      <div><h3 className="font-medium text-sm">Statement cycle</h3><p className="mt-1 text-xs leading-relaxed text-muted-foreground">FlowSight uses these dates to connect card purchases to the right future cash payment.</p></div>
      <div className="grid sm:grid-cols-2 gap-3"><Field label="Statement closes (day)" value={closingDay} onChange={setClosingDay} type="number" placeholder="15" /><Field label="Next payment due" value={dueDate} onChange={setDueDate} type="date" /></div>
      <Field label="Unpaid statement balance" value={statementBalance} onChange={setStatementBalance} type="number" placeholder="Optional" required={false} />
      <p className="text-[11px] text-muted-foreground">FlowSight adds charges from the current cycle. If no card transactions exist yet, this amount is used as the initial payment estimate.</p>
    </div>}
    {type === "credit_card" && !isNew && existingCard && <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-4">
      <div><h3 className="font-medium text-sm">Next card payment</h3><p className="text-xs text-muted-foreground mt-1">FlowSight combines the unpaid statement with card charges already recorded in the current cycle.</p></div>
      <div className="grid sm:grid-cols-2 gap-3"><Field label="Unpaid statement balance" value={statementBalance} onChange={setStatementBalance} type="number" placeholder="0.00" /><Field label="Next payment due" value={dueDate} onChange={setDueDate} type="date" /></div>
