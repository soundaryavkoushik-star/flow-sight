        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Import CSV">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
            ...
            {stage === "safety-buffer" && <div className="space-y-5">
              <div><p className="text-xs uppercase tracking-[0.14em] text-primary font-mono mb-1">Safety buffer</p><h3 className="text-xl font-semibold">How much would you like to keep untouched?</h3><p className="text-sm text-muted-foreground mt-2">FlowSight protects this amount before calling money safe to spend. You can change it anytime.</p></div>
              ...
              <p className="rounded-xl bg-muted/50 p-3 text-xs leading-relaxed text-muted-foreground">{safetyBuffer === "0" ? "A $0 buffer is still a configured choice: FlowSight will only protect against a negative balance." : "Your Safe to Spend amount will reflect this buffer."}</p>
              {error && <p className="text-sm text-destructive">{error}</p>}
