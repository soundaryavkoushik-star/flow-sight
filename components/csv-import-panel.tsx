...
import { detectAmountColumns, findHeader, normalizeDate, parseCsv, parseMoney, suggestRecurring, type CsvDateOrder } from "@/lib/csv/parse"
...
export function CsvImportPanel({ autoOpen = false, accounts = [] }: { autoOpen?: boolean; accounts?: TransactionAccountOption[] }) {
  ...
  const normalized = useMemo(() => rawRows.map((row, index) => {
    ...
    const signed = amountMode === "signed" ? parseMoney(row[Number(amountColumn)] ?? "") : null
  ...
  }), [rawRows, dateColumn, descriptionColumn, amountMode, amountColumn, debitColumn, creditColumn, dateOrder])
