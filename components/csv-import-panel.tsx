...
type Stage = "choose" | "assist" | "map" | "review" | "recurring" | "card-payment" | "income-pattern" | "done"
...
export function CsvImportPanel({ autoOpen = false, accounts = [], initialAccountId, existingRecurring = [], incomePatternEstablished = false, dismissedRecurringKeys = [] }: { autoOpen?: boolean; accounts?: TransactionAccountOption[]; initialAccountId?: string; existingRecurring?: ExistingRecurringItem[]; incomePatternEstablished?: boolean; dismissedRecurringKeys?: string[] }) {
  ...
  const close = useCallback((clearImportParameter = true) => {
    ...
    setError(null)
    if (autoOpen && clearImportParameter) router.replace("/app/transactions")
  }, [autoOpen, router])
  ...
  async function importReviewedTransactions() {
    ...
    setReconciliationChoices(Object.fromEntries(groups.map((group) => [group.id, group.status === "new" ? "add" : "keep"])))
    setStage(found.length > 0 ? "recurring" : paymentProposal ? "card-payment" : incomePatternEstablished ? "done" : "income-pattern")
    router.refresh()
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Import CSV">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
            ...
            {stage === "card-payment" && cardPaymentProposal && <div className="space-y-5">
              ...
              {error && <p className="text-sm text-destructive">{error}</p>}
              <div className="flex justify-between"><Button variant="outline" onClick={() => setStage(incomePatternEstablished ? "done" : "income-pattern")}>Not now</Button><Button disabled={working || !importedAccountId} onClick={async () => {
