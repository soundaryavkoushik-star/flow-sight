...
export function ActionToast({ message, tone = "success", action, onAction, onDismiss }: { message: string; tone?: "success" | "info" | "error"; action?: string; onAction?: () => void; onDismiss?: () => void }) {
  const toneClass = tone === "error" ? "border-destructive/25 bg-[oklch(var(--fs-red-bg))]" : tone === "info" ? "border-border bg-muted" : "border-[oklch(var(--fs-green))]/25 bg-[oklch(var(--fs-green-bg))]"
