...
export const metadata: Metadata = { title: "Features", description: "Explore Cusp’s cash-flow forecasting, recurring-pattern detection, transfer matching, credit-card timing, and Scenario Planner." }
...
function RecurringPreview() {
  ...
  return <div className="rounded-2xl border border-border bg-card p-5 sm:p-6">
    ...
    <div className="mt-5 space-y-2">
      {items.map(([name, type, frequency, amount, confidence]) => <div key={name} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl border border-border px-3.5 py-3">
        <FinancialEventIcon name={name} amountCents={type === "income" ? 1 : -1} confidence={confidence} />
        <div><div className="flex flex-wrap items-center gap-1.5"><p className="text-sm font-medium text-foreground">{name}</p><ConfidencePill confidence={confidence} /></div><p className="mt-0.5 text-[10px] text-muted-foreground">{frequency} · {type === "income" ? "Income" : "Bill"}</p></div>
