function SkeletonBlock({ className = "", style }: { className?: string; style?: React.CSSProperties }) {
  return <div className={`rounded-xl bg-[#EDE8DF] ${className}`} style={style} aria-hidden="true" />
}

export default function DashboardLoading() {
  return (
    <main className="mx-auto w-full max-w-6xl animate-pulse px-4 py-8 sm:px-6 lg:px-8" aria-busy="true" aria-label="Loading your financial briefing">
      ...
      <section className="mb-5 rounded-3xl border border-[#E7DDD1] bg-white p-5 sm:p-7">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[["w-28", "w-32"], ["w-24", "w-28"], ["w-40", "w-28"], ["w-36", "w-44"]].map(([labelWidth, valueWidth], index) => (
          <div key={index} className="min-h-36 rounded-2xl border border-[#E7DDD1] bg-white p-5">
      ...
      <section className="mt-5 rounded-3xl border border-[#E7DDD1] bg-white p-5 sm:p-7">
        ...
        <div className="mt-8 flex h-52 items-end gap-2 border-b border-l border-[#E7DDD1] px-3 pb-3">
